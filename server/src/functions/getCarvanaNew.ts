...
export async function getCarvanaNew(req:Request, res:Response){
    try {
      // @ts-ignore
        ...
        const totalPages = await page.$eval('[data-qa="total-pages"]', (item:any) => item.textContent);
        ...
        while (currentPageNum < 201) {
          ...
          try {
            const carVanaData = await page.$$eval(".result-tile", (carCard:any) => {
              return carCard.map((item:any) => {
