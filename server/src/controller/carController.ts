...
import { CarDataType, allCarData } from "../data/Data.js";
...
import { FilterData, ItemDetails, MakeAndModalStates, SearchCars, SimilarCars } from "./controllerTypes/carTypes.js";
import { PriceAndMileage, FilterFuncType, NameANdModalType } from "./controllerTypes/carTypes.js";
env(true);
...
export async function getCarVana(req: Request, res: Response) {
...
export async function filterData(req:FilterData, res:Response) {
  try {
    ...
    for (let items in makeAndModalStates) {
      if (makeAndModalStates[items as keyof MakeAndModalStates] === true) {
    ...
    const filters = {
      price: (price:number) => price >= lowPrice && price <= highPrice,
      mileage: (mileage:number) => mileage >= lowMileage && mileage <= highMileage,
      name_modal: newArr,
    };
    ...
    function filterPlainArray(array:Array<CarDataType>, filters:{
      ...
      return array.filter((item) => {
        ...
        return filterKeys.every((key) => {
          ...
          if (typeof filters[key as keyof FilterFuncType] !== "function" && Array.isArray(filters[key as keyof FilterFuncType]) && filters[key as keyof FilterFuncType].length) {
            return filters[key as keyof NameANdModalType].find((filter:string) => item[key as keyof NameANdModalType].includes(filter));
          } else if (typeof filters[key as keyof FilterFuncType] !== "function") {
            ...
            return filters[key as keyof PriceAndMileage](item[key as keyof {price:number,mileage:number}]);
          }
        });
      });
    }
...
export async function relatedCarData(req:Request, res:Response) {
  try {
...
// Search Data
export async function searchFunction(req:SearchCars, res:Response) {
  try {
    let data = allCarData;
    const { searchValue } = req.body;

    function searchGeneretor(mockData:Array<CarDataType>, searchVal:string) {
...
// Item Details
export async function getOneCar(req:ItemDetails, res:Response) {
  try {
    const { id } = req.body;
    if(typeof parseFloat(id) !== "number") return res.status(400).json({msg:"Id of car is not a number"})
    let data = allCarData.find((item) => item.index === parseFloat(id));
...
// Similar Cars
export async function similarCars(req:SimilarCars, res:Response) {
  try {
    const { id } = req.body;
    if(typeof parseFloat(id) !== "number") return res.status(400).json({msg:"Id of car is not a number"})
    const findOne = allCarData.find((item) => item.index === parseFloat(id));

    function shuffle(array:Array<CarDataType>) {
