...
export async function forgot_Password(req, res) {
    try {
        ...
        const token = jwt.sign({ email: email }, process.env.SECRET, { expiresIn: "65s" });
...
export async function check_Otp_Value(req, res) {
    try {
        const { code } = req.body;
        ...
        if (data.rows[0]) {
            const token = jwt.sign({ email: data.rows[0].email }, process.env.SECRET, { expiresIn: "10m" });
...
export async function reset_Password(req, res) {
    try {
        ...
        if (!userEmail)
            return res.status(400).json({ msg: "Not authorized, your session has expired" });
        ...
        if (loginTable.rows[0]) {
            ...
            if (isMatch)
                return res.status(400).json({ msg: "This password is not different from your current password" });
...
export function practiceToken(req, res) {
    try {
        const decode = jwt.verify("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6ImNodWt3YWdvbjFAZ21haWwuY29tIiwiaWF0IjoxNzE3MTQ1MDcyLCJleHAiOjE3MTcxNDUxMzd9.mvwhdi1gEZHAKOpeGlE161hbK501nxeFdO7bS1XqZsc", process.env.SECRET);
        res.send(decode);
    }
    catch (e) {
        console.log(e);
