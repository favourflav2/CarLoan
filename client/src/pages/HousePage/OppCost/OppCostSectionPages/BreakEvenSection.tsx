...
export interface IBreakEvenSectionProps {
  breakEvenOppCost: FTVOppCost;
  selectedGoal: HouseObjWithFormattedData;
  monthlyPayment: HouseMonthlyPayment;
}

export default function BreakEvenSection({ breakEvenOppCost, selectedGoal, monthlyPayment }: IBreakEvenSectionProps) {
  ...
  return (
    <div className="w-full min-h-[600px] rounded-lg shadow-[0px_6px_15px_0px_#00000024] dark:shadow-[0px_6px_15px_0px_#271d17] flex flex-col p-6 bg-inherit ">
      ...
      <div className="flex flex-col w-auto h-auto mb-5">
        <h1 className="text-[26px] font-bold">Break Even Point Per Month</h1>
        <h1 className="text-[13px] italic">
          * This scenario explains how much money you would have if you invested the break even point per month instead of paying the total cost of home ownership every month.
        </h1>
      </div>
      ...
      <div className="w-full flex sm:flex-row flex-col items-center justify-center sm:mt-0 mt-4">
        ...
        <div className="w-auto flex flex-col h-auto sm:ml-[20px]  sm:mt-0 mt-6">
          ...
          <div className="w-auto flex items-center justify-between mb-1">
            ...
            <p className="xl:ml-[40px]  text-[13px] font-bold italic ml-[38px]">{breakEvenOppCost.totalPrincipal}</p>
          </div>
          ...
          <div className="w-auto flex items-center justify-between">
            ...
            <p className="xl:ml-[40px]  text-[13px] font-bold italic ml-[38px]">{breakEvenOppCost.totalInterst}</p>
          </div>
        </div>
      </div>
          ...
          <div className="w-full h-full flex flex-col  ">
            <ul className=" list-inside list-disc">
              ...
              <li className="text-[14px] underline mb-1 dark:text-chartGreen text-green-700 ">
                <span className="font-bold italic">{getBreakEvenNumber(selectedGoal).resultFormatted}/mo</span> each payment gets invested and receives the benefit of compounding interest
              </li>
              <Tooltip title="The Stock/Bond/Investment Expected Return rate is what is being used to help estimate how much money you would have if you invested in the market." placement="top-start">
                <li className="text-[14px] underline mb-1 cursor-pointer">
                  In {selectedGoal.term} years investing the Break Even... due to compounding interest you would have accumulated{" "}
                  <span className="font-bold italic">{breakEvenOppCost.highestNum}</span>
                  <span className="text-[17px]">*</span>
                </li>
              </Tooltip>
              <li className="text-[14px] underline mb-1 dark:text-chartGreen text-green-700">
                Total interest accrued <span className="font-bold italic">{breakEvenOppCost.totalInterst}</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      <div className="w-full h-auto sm:flex hidden items-center justify-center my-4 ">
        <h1 className="text-[15px]">
          {" "}
          {futureValueOfPresentValue(selectedGoal.price, selectedGoal.appreciation, selectedGoal.term) > breakEvenOppCost.highestNumNoFormat ? (
            <>
              <span className="font-bold italic">{USDollar.format(Number(futureValueOfPresentValue(selectedGoal.price, selectedGoal.appreciation, selectedGoal.term)))}</span>{" "}
              <span className="text-[20px] mx-2 font-bold">{">"}</span> <span className="font-bold italic">{breakEvenOppCost.highestNum}</span>
            </>
          ) : (
            <>
              Investing after {selectedGoal.term} years: <span className="font-bold italic">{breakEvenOppCost.highestNum}</span> <span className="text-[20px] mx-2 font-bold">{">"}</span> Home
              appreciation after {selectedGoal.term} years:{" "}
              <span className="font-bold italic">{USDollar.format(Number(futureValueOfPresentValue(selectedGoal.price, selectedGoal.appreciation, selectedGoal.term)))}</span>
            </>
          )}
        </h1>
      ...
      <div className="w-full flex sm:hidden flex-col justify-center items-center my-4 h-auto">
        <h1 className="text-[14px]">
          {futureValueOfPresentValue(selectedGoal.price, selectedGoal.appreciation, selectedGoal.term) > breakEvenOppCost.highestNumNoFormat ? (
            ...
            <span>
              Investing: <span className="font-bold italic">{breakEvenOppCost.highestNum}</span>
            </span>
          )}
        </h1>
      ...
      <div className="w-full flex flex-col h-auto">
        ...
        {/* Investing */}
        <div className="w-full flex flex-col mb-4">
          <h1 className="sm:text-base text-[15px] font-bold underline mb-[2px]">Investing</h1>
          <p className="text-[13px] text-wrap">
            This is only works if you take the money that would be used in owning a home and decided to invest it. If done, you would reap the benefits of compounding interest. If you invested{" "}
            <span className="font-bold italic">{getBreakEvenNumber(selectedGoal).resultFormatted}</span> each month, and your expected return rate over the course of 30 years averaged{" "}
            <span className="font-bold italic">{selectedGoal.opportunityCostRate}%</span> annually you would have <span className="font-bold italic">{breakEvenOppCost.highestNum}</span>.
          </p>
        </div>
